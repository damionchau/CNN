...
def generate_response(prompt):
    ...
    completions = openai.Completion.create(
        engine=model_engine,
        prompt=prompt, #initiates prompt below in while statement
        max_tokens=20,
        n=1, #represents number of responses
        stop=None, #stops response under specific condition
        temperature=0.1, #varies responses creativity
    )
